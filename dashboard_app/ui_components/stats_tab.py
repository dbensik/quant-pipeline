...
class MetricsRenderer:
    """Renders key performance and risk metrics."""
    def render(self, stats: dict, risk_metrics: dict):
...
class ChartRenderer:
    """Renders various financial charts."""
    def render_returns_histogram(self, portfolio: pd.DataFrame):
    ...
    def render_drawdown_series(self, portfolio: pd.DataFrame):
    ...
    def render_strategy_diagnostics(self, diagnostics: pd.DataFrame):
        """
        Displays a chart of a strategy's internal state.
        """
...
class TradeLogRenderer:
    """Renders the execution/trade log."""
    def render(self, trade_log: pd.DataFrame):
        ...
        display_df["price"] = display_df["fill_price"].map("${:,.2f}".format)
        display_df["cost"] = display_df["total_cost"].map("${:,.2f}".format)
        
        st.dataframe(
            display_df[["date", "symbol", "direction", "quantity", "price", "cost"]],
...
class StatsTab:
    ...
    Refactored to separate rendering concerns into helper classes.
    """
    def __init__(self):
        self.metrics_renderer = MetricsRenderer()
        self.chart_renderer = ChartRenderer()
        self.log_renderer = TradeLogRenderer()
    ...
    def _render_stats_details(self, symbol: str, result: dict):
        ...
        # Use helper renderers
        self.metrics_renderer.render(stats, risk_metrics)

        st.divider()

        if diagnostics is not None and not diagnostics.empty:
            self.chart_renderer.render_strategy_diagnostics(diagnostics)
            st.divider()

        col1, col2 = st.columns(2)
        with col1:
            self.chart_renderer.render_returns_histogram(portfolio)
        with col2:
            self.chart_renderer.render_drawdown_series(portfolio)

        st.divider()
        if trade_log is not None and not trade_log.empty:
            self.log_renderer.render(trade_log)
        else:
